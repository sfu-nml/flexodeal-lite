  ...
  namespace Parameters
  {
    ...
    void MuscleProperties::declare_parameters(ParameterHandler &prm)
    {
      prm.enter_subsection("Materials");
      {
        ...
        prm.declare_entry("Fat fraction", "0.02",
                          Patterns::Double(),
    ...
    // @sect4{Output binary files}

    // This section implements the booleans that tell the solver
    // whether to output binary files or not. Because these files
    // are usually large in comparison to the other outputs, we
